...
AsyncClientClass aClientStream(ssl_client, getNetwork(network));
FirebaseApp app;
...
void initFirebase() {
  ...
  ssl_client.setInsecure(); // Bypass SSL cert verification (ESP32)

  // Inisialisasi app dengan NoAuth
...
void streamCallback(AsyncResult &aResult) {
  ...
  // Dapatkan path dan data
  RealtimeDatabaseResult &rtdbResult = aResult.to<RealtimeDatabaseResult>();
  String path = rtdbResult.event();
  String data = rtdbResult.data();

  Serial.printf("📡 Stream event - Path: %s, Data: %s\n", path.c_str(), data.c_str());

  // Parse kontrol dari web dashboard
  // Path akan berupa: /kipas, /solenoid, /buzzerMute
  if (path == "put") {
    String evPath = rtdbResult.dataPath();

    if (evPath == "/kipas" || evPath == "kipas") {
    ...
    else if (evPath == "/solenoid" || evPath == "solenoid") {
      int nilai = data.toInt();
    ...
    else if (evPath == "/buzzerMute" || evPath == "buzzerMute") {
      int nilai = data.toInt();
      buzzerMuted = (nilai == 1);
      if (buzzerMuted) {
...
void startStream() {
  ...
  Database.get(aClientStream, "/kontrol", streamCallback, true /* SSE stream */);
  streamStarted = true;
...
void bacaSensor() {
  ...
  if (suhu > SUHU_MAX) {
    ...
    alarmSuhuSent = false; // Reset flag kirim
  }
...
void kirimDataFirebase() {
  ...
  // Fallback ke NTP jika RTC tidak tersedia
  ...
  // ---- 1. Update data real-time (overwrite) ----
  String realtimeJson = "{";
  ...
  bool setOk = Database.set<object_t>(aClient, "/realtime", object_t(realtimeJson));
  if (setOk) {
    dataSuccessCount++;
  ...
  String pushResult = Database.push<object_t>(aClient, "/log", object_t(logJson));
  ...
  if (alarmSuhuAktif && !alarmSuhuSent) {
    ...
    alarmJson += "}";
    Database.push<object_t>(aClient, "/alarm", object_t(alarmJson));
    alarmSuhuSent = true;
...
void sendHeartbeat() {
  ...
  Database.set<object_t>(aClient, "/status", object_t(heartbeatJson));
}
...
void setup() {
  ...
  if (wifiConnected) {
    ...
    configJson += "}";
    Database.set<object_t>(aClient, "/config", object_t(configJson));
  }
